...
function renderMacroWorkspace() {
  ...
  const orderBook = `<article class="provider-card"><span>BTC offered for sale near market price</span><strong>Order-book depth provider needed</strong><p>${escapeHtml(macroSupplyData?.order_book?.message ?? 'Historical ask-side depth requires a commercial market-data source.')}</p><small>This is the narrower, immediately executable subset—not the full exchange reserve.</small></article>`;
  document.querySelector('#macroSupplyPane').innerHTML = `<div class="macro-section-heading"><div><h3>BTC market supply</h3><p>Shows the exchange-reserve estimate first, then stablecoin liquidity and the narrower live sell-side order book.</p></div></div><div class="supply-warning">${escapeHtml(macroSupplyData?.warning ?? 'Exchange custody inventory is not equivalent to coins offered for sale.')}</div><div class="macro-chart-grid">${exchangeCards}${stablecoin ? macroMetricCard(stablecoin, '#22c55e') : ''}${providerMessage}${orderBook}</div>`;

  const refreshWarnings = (data.refresh_errors ?? []).filter((warning) => !warning.startsWith('Glassnode exchange supply'));
  document.querySelector('#macroSources').innerHTML = `<p>${escapeHtml(data.methodology.warning)}</p><p><strong>Net liquidity:</strong> ${escapeHtml(data.methodology.net_liquidity_formula)}. <strong>Treasury countries:</strong> ${escapeHtml(data.methodology.country_warning)}</p><p><strong>Official sources:</strong> Federal Reserve/FRED, U.S. Treasury FiscalData, and Treasury International Capital. <strong>Crypto liquidity:</strong> CoinGlass reviewed exchange-reserve snapshot, DefiLlama stablecoin history, and Glassnode flow metrics when an API key is configured.</p>${refreshWarnings.length ? `<p><strong>Refresh warnings:</strong> ${escapeHtml(refreshWarnings.join(' | '))}</p>` : ''}<p><strong>Cycle Signals:</strong> ISM is a manual snapshot; copper, gold and oil are market proxies; DVOL is from Deribit.</p>`;
}
